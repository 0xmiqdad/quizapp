const quizData = [
  ...
  function startTimer() {
      ...
      timerInterval = setInterval(() => {
          console.log(`Time left: ${timeLeft}s`); // Debugging log
          if (timeLeft > 0) {
              timeLeft--;
              timerEl.innerText = `Time left: ${timeLeft}s`;
          } else {
              clearInterval(timerInterval);
              endQuiz("⏳ Time's up! You can no longer answer.");
          }
      }, 1000);
  }
  ...
  function loadQuiz() {
      deselectAnswers();
      feedbackEl.innerText = ""; // Clear feedback message
  ...
  submitBtn.addEventListener("click", () => {
      ...
      if (selectedAnswer) {
          if (selectedAnswer === quizData[currentQuiz].correct) {
              ...
              feedbackEl.innerText = "❌ Wrong answer! The correct answer was: " +
          ...
          setTimeout(() => {
              ...
              if (currentQuiz < quizData.length) {
                  ...
                  clearInterval(timerInterval);
                  endQuiz(`🎉 You answered ${score}/${quizData.length} questions correctly.`);
              }
          }, 1500); // Increased timeout for better readability
      } else {
          feedbackEl.innerText = "⚠️ Please select an answer!";
